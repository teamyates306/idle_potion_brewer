        ...
        {entries.length === 0 ? (
          ...
          <>
            <div className="space-y-2">
              {entries.map(([hash, count]) => {
                const d = describeFromHash(hash, cfg.ingredients, cfg.formulas);
                if (!d) return null;
                return (
                  <div key={hash} className="flex items-center gap-2 rounded-lg bg-slate-800/60 p-3">
                    <button
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="truncate font-medium text-purple-200">{d.name}</div>
                        <div className="text-xs text-slate-400">×{count} · 🪙 {fmt(d.value)} each</div>
                      </div>
                    </button>
                    <div className="flex shrink-0 gap-1">
                      <button onClick={() => sellPotion(hash, 1)} className="rounded bg-slate-700 px-2 py-1 text-xs hover:bg-slate-600">
                        Sell 1
                      </button>
                      <button onClick={() => sellPotion(hash, count)} className="rounded bg-purple-600 px-2 py-1 text-xs text-white hover:bg-purple-500">
                        All
                      </button>
                    </div>
                  </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ingredientIds.map((id) => {
                    ...
                    if (!ing) return null;
                    return (
                      <span key={id} className="flex items-center gap-1 rounded-full border border-slate-700 bg-slate-800 px-2 py-0.5 text-xs text-slate-300">
