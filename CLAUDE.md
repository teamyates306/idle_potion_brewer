...
### Graphics renderer modes (GPU / battery)
`settingsStore.graphicsMode` (Settings → "Optimised renderer", default `optimized`; `legacy` kept for A/B comparison and slated for removal) selects between two visually identical render paths, read via `useOptimizedGfx()` and mirrored as `<html class="gfx-legacy">` for CSS. Optimised mode: **pre-tinted sprites** instead of per-frame `filter: hue-rotate()` (WorkerArt / MachineArt; MachineArt rotates its SVG overlay colours with `util/hueRotate.ts`, the exact CSS matrix), **one merged day/night tint layer** (`--dn-tint`, exact source-over of the warm+cool layers), **motes + lantern flicker driven by `engine/ambientClock.ts`** (one 30 Hz JS clock evaluating the same keyframes, instead of ~40 vsync CSS animations), **no `backdrop-filter`** (CSS rule in index.css), and **off-screen cauldron loops paused** via IntersectionObserver. Pre-tinted sheets live in `public/sprites/tinted/` and are generated by `npx tsx scripts/pretintSprites.ts` (`--check` verifies they're current) — re-run it whenever a worker/machine sheet or the hue tables (`WorkerArt.HUE_SHIFTS`, `Workshop.MACHINE_HUE`) change. Anything continuous you add to the scene should run off `subscribeAmbient` rather than an infinite CSS animation: every running compositor animation forces a GPU frame at the display refresh rate.
